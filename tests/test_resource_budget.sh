...
' _ "$SCRIPT" >/tmp/codex-supervisor-node-budget.out 2>/tmp/codex-supervisor-node-budget.err; then
  echo "invalid NODE_MAX_PANES should sanitize to default cap and reject over-cap starts" >&2
  exit 1
fi
