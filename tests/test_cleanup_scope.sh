...
CODEX_SUPERVISOR_PERIODIC_CLEANUP_SECS=0 \
  bash -c '
    ...
    cleanup_path_in_project_scope "$2/projects/current-proj-worker"
    ! cleanup_path_in_project_scope "$2/projects/other-proj-worker"
    ...
    [[ ! -e "$2/private-tmp/current-proj-temp" ]]
    [[ -e "$2/private-tmp/other-proj-temp" ]]
    [[ ! -e "$2/superpowers/worktrees/current-proj/old-worker" ]]
    [[ -e "$2/superpowers/worktrees/other-proj/old-worker" ]]
    [[ ! -e "$2/projects/current-proj-worker" ]]
    [[ -e "$2/projects/other-proj-worker" ]]

    PRUNE_WORKTREE_AGE_HOURS=-1
    cmd_cleanup
    [[ ! -e "$2/projects/current-proj/.next" ]]
    [[ -e "$2/projects/other-proj/.next" ]]
    [[ -e "$2/projects/other-proj-worker" ]]
  ' _ "$SCRIPT" "$TMPDIR"
