...
[[ "$DASHBOARD_PORT" =~ ^[0-9]+$ && "$DASHBOARD_PORT" -gt 0 ]] || DASHBOARD_PORT=7777
[[ "$DASHBOARD_LINES" =~ ^[0-9]+$ && "$DASHBOARD_LINES" -gt 0 ]] || DASHBOARD_LINES=12
[[ "$DASHBOARD_REFRESH" =~ ^[0-9]+([.][0-9]+)?$ ]] || DASHBOARD_REFRESH=0.2
...
try:
    ...
    if MAX_SOURCE_HASH_BYTES < 1024 or MAX_SOURCE_HASH_BYTES > 200000000:
